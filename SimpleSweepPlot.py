def SimpleSweepPlot(datadir, search_4Snums=False, Snums='', verbose=False,     \
standdev=True, std_num=1, do_title=True, show_plot=False, save_plot=True,      \
do_eps=True, show_fastIV=True, show_unpumped=True, find_lin_mVuA=False,        \
linif=0.3,                                                                     \
...
der2_int=1, do_der2_conv=True, der2_min_cdf=0.95, der2_sigma=0.05):
    ...
    for Snum in Snums:
        ...
        if verbose:
            print "ploting for Ynum: " + str(Snum)
	...
	if platform == 'win32':
        ...
        if standdev:
            ## AXIS 1
            # Positive sigma
            ax1.plot(mV_mean, uA_mean+(uA_std*std_num), color=ax1_color,       \
            ...
            ## AXIS 2
            # Positive sigma
            ax2.plot(mV_mean, (TP_mean+(TP_std*std_num))*tp_scale,             \
            color=ax2_color, linewidth=1, ls='dotted')
            # Negative sigma
            ax2.plot(mV_mean, (TP_mean-(TP_std*std_num))*tp_scale,             \
            color=ax2_color, linewidth=1, ls='dotted')
        ...
        if find_lin_mVuA:
            ...
            linfit(mV_mean, uA_mean, linif, der1_int, do_der1_conv,            \
            der1_min_cdf, der1_sigma, der2_int, do_der2_conv, der2_min_cdf,    \
            der2_sigma, verbose)
...
################ HERE #######################
        ...
        if do_title:
            ...
            K_val, magisweep, magiset, magpot, meanmag_V, stdmag_V, meanmag_mA,\
stdmag_mA, sisisweep, sisiset, UCA_volt, meanSIS_mV, stdSIS_mV, meanSIS_uA,    \
stdSIS_uA, meanSIS_tp, stdSIS_tp, SIS_pot, del_time, LOfreq, IFband            \
= getproparams(paramsfile)
            title_str = "Magnet current: " + str('%2.3f' % meanmag_mA) +       \
"mA,  SIS current at " + str('%2.3f' % meanSIS_mV) +                           \
"mV (" + str('%2.3f' % stdSIS_mV) +"): " + str('%2.3f' % meanSIS_uA) +         \
"uA (" + str('%2.3f' % stdSIS_uA) +")"
            plt.title(title_str)
...
#
#setnum = 2
#search_4Snums = False     # defualt is True
#Snums         = ['00001'] # defualt is empty string ''    
#verbose       = True      # default is False
#standdev      = True      # polt the standet deviation of the plotted curves, defualt is True
#std_num       = 3         # number of standard deviations to plot, defualt is '1'
#
#do_title      = True      # show plot title, defult is True
#show_plot     = True     # display the plot on screen (not a good idea for 100's of plots), defualt is false       
#save_plot     = True      # save the plot to a file, default is True
#do_eps        = True      # True save the plot as an .eps file, False saves the plot as a .png file, defualt is True
#show_fastIV   = True      # Show the fast IV curve if it is found, defualt is True
#show_unpumped = True      # Show the unpumped (LO off) IV curve if it is found, defualt is True
#
