# Y factor calculator and Plot Maker created October 29, 2013
...
# corrisponding to (LNA_off_300K_Y##_L_sweep####.csv, 
# UCA#.##_300K_Y##_L_sweep####.csv, UCA#.##_077K_sweep####.csv).
# Sweeps can be wide (W) or narrow (N); this is the Letter after the Y number.
# At least 3 files must be present LNA off, hot, and cold. Also I expect the 
# the files to either be all (W) or (N), but the code is made to either both.
...
for n in range(len(Ynums)): # start Y factor loop
    ...
    #####
    status=False
    wide_off_csv, wide_hot_csv, wide_cold_csv, narrow_off_csv, narrow_off_csv, narrow_hot_csv, narrow_cold_csv, UCA_val, status = Y_jobs(Y_csvfiles)
    if status == False:
    ...
    if show_plots:
        if test_plots == True:
            ...
            if (not wide_missing):
                ...
                plt.plot(wide_off_mV, wide_off_uA         , linewidth=wide_lineW, color = 'yellow', label='wide off IV')
                plt.plot(wide_off_mV, wide_off_tp*tp_scale, linewidth=wide_lineW, color = 'orange', label='wide off TP')
        
                plt.plot(wide_hot_mV, wide_hot_uA         , linewidth=wide_lineW, color = 'red'   , label='wide hot IV ')
                plt.plot(wide_hot_mV, wide_hot_tp*tp_scale, linewidth=wide_lineW, color = 'purple', label='wide hot TP ')
    
                plt.plot(wide_cold_mV, wide_cold_uA        , linewidth=wide_lineW, color = 'dodgerblue', label='wide cold IV ')
                plt.plot(wide_cold_mV, wide_cold_tp*tp_scale, linewidth=wide_lineW, color = 'blue'      , label='wide cold TP ')
            ...
            if ((do_yfactor==True) and (do_regrid == True) and (mono_switcher==True)):
                ...
                plt.text(0.5, 40, "Y factor: "+str('%2.3f' % max_Yfactor)+" at "+str('%2.3f' % mV_max_Yfactor)+" mV", fontsize=10)
                plt.plot([mV_max_Yfactor,mV_max_Yfactor], [yaxis_min,yaxis_max], linewidth=1, color = 'red', label='max Y factor')
                plt.plot([0,4], [max_Yfactor*Y_scale,max_Yfactor*Y_scale], linewidth=1, color = 'red')
            ...
            if ((do_yfactor==True) and (do_regrid == True) and (mono_switcher==True)):
                plt.title("Y-Factor:"+str('%2.3f' % max_Yfactor)+" UCA:"+UCA_val)
            else:
                plt.title("Y-Factor UCA:"+UCA_val)
        
            plt.show()
            time.sleep(1)
            if save_plots:
                savefig(dir+'plots/'+Ynums[n]+".png")
