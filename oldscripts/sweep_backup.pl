...
$socket = new IO::Socket::INET (PeerHost => '192.168.1.101', PeerPort => '9001', Proto => 'tcp') or die "ERROR connecting to server : $!\n";
$socket->send($sweepcommand);
sleep(7);  # increase w/ number of points, or loop w/ recv() until you get all data
open(FILE, '>sweep.dat');
$socket->recv($data,10240);
print FILE $data;
$socket->close();
close(FILE);
...
$scalefactor = ($sortedcurrent[-1]/$sortedpwr[-1])*4.0;
$i=0;
foreach $entry (@power)
{
    $power[$i]=$entry*=$scalefactor-65.0;
    $i++;
}
